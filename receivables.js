// Receivables Intelligence Module v24.0
...
function sectionTabs(){
  const root=$('recvSectionTabs');if(!root)return;
  const t=[['overview','Overview'],['aging','Aging Analysis'],['agingDetail','Aging Analysis Detail'],['customers','Top Customers'],['customerMovement','Over 180 Movement'],['movement','Movement Analysis'],['collections','Collections Performance']];
  root.innerHTML=t.map(x=>`<button class="recv-subtab ${S.section===x[0]?'active':''}" data-s="${x[0]}">${x[1]}</button>`).join('');
  root.querySelectorAll('[data-s]').forEach(b=>b.onclick=async()=>{
    S.section=b.dataset.s;sectionTabs();
    if(S.section==='collections'&&!matrix('Coll vs Target').length&&typeof window.loadGoogleSheetScope==='function'){
      const c=$('recvContent');if(c)c.innerHTML='<div class="empty">Loading Collection Performance…</div>';
      try{
        await window.loadGoogleSheetScope('collections',{force:false});
        applyPayload(window.GOOGLE_SHEET_RAW_PAYLOAD);
      }catch(e){
        if(c)c.innerHTML='<div class="empty">Could not load Collection Performance: '+esc(e.message)+'</div>';
      }
    }else content();
  });
}
...
function over180MovementRecords(){
  ...
  MOVEMENT_CONFIG.currentBlocks.forEach(block=>{
    ...
    previous.forEach(r=>{
      ...
      ensure(k,{customer:r.customer,division:r.division||'Unassigned'}).previous+=(r.over180||0)*factor;
    });
    current.forEach(r=>{
      const k=key(r.code,r.customer,r.division);
      ensure(k,{customer:r.customer,division:r.division||'Unassigned'}).current+=(r.over180||0)*factor;
    });
...
function renderOver180Movement(){
  ...
  const cards=`<div class="grid kpis recv-kpis">
    ...
    ${kpi('Customers',fmt(all.length),'With previous or current >180 balance')}
  </div>`;
  ...
  const table=`<div class="card panel recv-panel">
    <div class="panelhead"><div>
      ...
      <p class="hint">Previous period from DR-Last Period · Current period from live DR entity sheets</p>
    </div></div>
    <div class="recv-table-wrap">
      <table class="recv-table recv-customer-table">
        ...
        <tbody>${rows||'<tr><td colspan="8" class="empty">No over-180 customer movement detected.</td></tr>'}</tbody>
      </table>
    </div>
  </div>`;
  ...
  const chart=`<div class="card panel recv-panel">
    <div class="panelhead"><div><h3>Largest Movements</h3>
      <p class="hint">Green = improvement · Red = deterioration</p></div></div>
    <div class="recv-bars">${top.filter(r=>Math.abs(r.movement)>.5).map(r=>`
      <div class="recv-bar-row">
        <div class="recv-bar-label">${esc(r.customer)}</div>
        <div class="recv-bar-track"><div class="recv-bar-fill" style="width:${Math.max(2,Math.abs(r.movement)/max*100)}%"></div></div>
        <div class="recv-bar-value ${over180StatusClass(r.status)}">${fmt(r.movement)}</div>
      </div>`).join('')||'<div class="empty">No movements to chart.</div>'}
    </div>
  </div>`;
...
function renderAgingDetail(rs){
  const ccy=currency();
  const sorted=rs.slice().sort((a,b)=>(b.total||0)-(a.total||0));
  const head=['Customer','Entity','Division','Total','0–30','31–60','61–90','91–120','121–150','151–180','181–210','211–240','241–270','271–300','301–330','331–365','366–730','>731'];
  const body=sorted.map(r=>{
    const f=S.entity==='GROUP'?r.fx:1;
    const vals=BK.map(k=>(r.buckets[k]||0)*f);
  ...
  return `<div class="card panel recv-panel">
    <div class="panelhead"><div>
      <h3>Aging Analysis Detail</h3>
      <p class="hint">${sorted.length} customer balances · ${esc(ccy)} · all aging brackets</p>
    </div></div>
    <div class="recv-table-wrap">
      <table class="recv-table recv-aging-detail-table">
        <thead><tr>${head.map(h=>`<th>${h}</th>`).join('')}</tr></thead>
        <tbody>${body||'<tr><td colspan="18" class="empty">No aging detail detected.</td></tr>'}</tbody>
      </table>
    </div>
  </div>`;
}
...
function jsonp(url){return new Promise((res,rej)=>{const cb='recvJsonp_'+Date.now()+'_'+Math.random().toString(36).slice(2),sc=document.createElement('script'),sep=url.includes('?')?'&':'?',tm=setTimeout(()=>{done();rej(new Error('Receivables request timed out after 60 seconds.'))},60000);function done(){clearTimeout(tm);try{delete window[cb]}catch(_){window[cb]=undefined}if(sc.parentNode)sc.parentNode.removeChild(sc)}window[cb]=d=>{done();res(d)};sc.onerror=()=>{done();rej(new Error('Could not load Google Sheet endpoint.'))};sc.src=url+sep+'callback='+encodeURIComponent(cb)+'&t='+Date.now();document.body.appendChild(sc)})}
...
async function refresh(o){
  ...
  try{
    if(typeof window.loadGoogleSheetScope==='function'){
      await window.loadGoogleSheetScope('receivables',{force:!o.silent});
      if(S.section==='collections')await window.loadGoogleSheetScope('collections',{force:!o.silent});
      applyPayload(window.GOOGLE_SHEET_RAW_PAYLOAD);
      return;
    }
    const url=api();
    ...
    const payload=await jsonp(url+sep+'scope=receivables');
    window.GOOGLE_SHEET_RAW_PAYLOAD=payload;
    applyPayload(payload);
  }catch(e){
    if(root)root.innerHTML='<div class="empty">Could not load receivables: '+esc(e.message)+'</div>';
  }
}
function wrap(){
...
function boot(){
  ...
  window.addEventListener('googleSheetPayloadReady',function(e){
    const p=e&&e.detail?e.detail:window.GOOGLE_SHEET_RAW_PAYLOAD;
    if(p&&p.sheets)applyPayload(p);
  });
  setTimeout(()=>{
    if(typeof window.loadGoogleSheetScope==='function'){
      refresh({silent:true});
    }else if(window.GOOGLE_SHEET_RAW_PAYLOAD){
      applyPayload(window.GOOGLE_SHEET_RAW_PAYLOAD);
    }
  },500);
}
