...
function hmap(headers){const m={};headers.forEach((h,i)=>{const n=norm(h);if(!n)return;if(m.customer==null&&(n.includes('accountname')||n==='customername'||n==='customer'))m.customer=i;if(m.code==null&&(n.includes('accountcode')||n==='account'))m.code=i;if(m.division==null&&(n==='dimension'||n==='division'||n==='div'))m.division=i;if(m.total==null&&(n.includes('outstandingamount')||n==='totalreceivable'||n==='total'))m.total=i;[
...
function aging(a){const c=currency(),labels={'0_30':'0–30','31_60':'31–60','61_90':'61–90','91_120':'91–120','121_150':'121–150','151_180':'151–180','181_210':'181–210','211_240':'211–240','241_270':'241–270','271_300':'271–300','301_330':'301–330','331_365':'331–365','366_730':'366–730','gt731':'>731'},items=Object.keys(labels).map(k=>({label:labels[k],value:a.buckets[k]})),dr=Object.entries(a.divisions).sort((x,y)=>y[1].total-x[1].total).map(([d,x])=>`<tr><td class="rowhead">${esc(d)}</td><td class="num">${fmt(x.total)}</td><td class="num">${fmt(x.over90)}</td><td class="num">${fmt(x.over180)}</td><td class="num">${pct(x.total?x.over90/x.total*100:0)}</td></tr>`).join('');return`<div class="card panel recv-panel"><div class="panelhead"><div><h3>Aging Distribution</h3></div></div>${bars(items,c)}</div><div class="card panel recv-panel"><div class="panelhead"><div><h3>Division Aging Summary</h3></div></div><div class="recv-table-wrap"><table class="recv-table"><thead><tr><th>Division</th><th>Total</th><th>&gt;90 Days</th><th>&gt;180 Days</th><th>&gt;90 %</th></tr></thead><tbody>${dr}</tbody></table></div></div>`}
...
function content(){const root=$('recvContent');if(!root)return;const rs=rows(),a=aggregate(rs);if(!rs.length){root.innerHTML='<div class="card panel"><div class="empty">No receivable rows detected. Confirm the ERP tabs are exported and contain Account Name, Dimension, Outstanding Amount and aging headers.</div></div>';return}if(S.section==='aging')root.innerHTML=aging(a);else if(S.section==='customers')root.innerHTML=`<div class="recv-two">${topTable(rs,'total','Top 10 Outstanding Customers')}${topTable(rs,'over180','Top Outstanding Customers Over 180 Days')}</div>`;else if(S.section==='movement')root.innerHTML=optional('movement');else if(S.section==='collections')root.innerHTML=optional('targets');else root.innerHTML=overview(rs,a)}
